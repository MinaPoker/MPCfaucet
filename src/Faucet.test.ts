...
async function main({ withVesting }: { withVesting: boolean }) {
    ...
    tx = await Mina.transaction(
        ...
        async () => {
            ...
            await dex.supplyLiquidityBase(UInt64.from(10_000), UInt64.from(10_000), mainUser);
        }
    );
    await tx.prove();
    tx.sign([feePayerKey]);
    ...
    tx = await Mina.transaction(addresses.user2, async () => {
        ...
        await dex.supplyLiquidityBase(UInt64.from(100), UInt64.from(100), mainUser);
    });
    await tx.prove();
    tx.sign([keys.user2]);
    ...
    tx = await Mina.transaction(addresses.user, async () => {
        await dex.supplyLiquidityBase(UInt64.from(1e9), UInt64.from(1e9), mainUser);
    });
    await tx.prove();
    tx.sign([keys.user]);
    await expect(tx.send()).rejects.toThrow(/Overflow/);
    ...
    if (withVesting) {
        ...
        tx = await Mina.transaction(addresses.user, async () => {
            await dex.redeemLiquidity(UInt64.from(USER_DL), mainUser);
        });
        await tx.prove();
        tx.sign([keys.user]);
    ...
    tx = await Mina.transaction(addresses.user2, async () => {
        await dex.redeemLiquidity(UInt64.from(1n), mainUser);
    });
    await tx.prove();
    ...
    tx = await Mina.transaction(addresses.user, async () => {
        await dex.swapX(UInt64.from(USER_DX), mainUser);
    });
    await tx.prove();
    await tx.sign([keys.user]).send();
    [oldBalances, balances] = [balances, getTokenBalances()];
